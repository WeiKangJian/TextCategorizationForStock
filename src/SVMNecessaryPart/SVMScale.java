...
public class SVMScale {
	...
	private void output_target(DataOutputStream DataOutput,double value)throws IOException
	{
		if(y_scaling)
		{
			if(value == y_min)
	...
	private void output(DataOutputStream DataOutput,int index, double value)throws IOException
	{
		...
		{
			//输出到文件
			DataOutput.writeBytes(index + ":" + value + " ");
	...
	public void run(String argv[]) throws IOException
	{
		...
		String scale_data_filename=null;
		...
		for(i=0;i<argv.length;i++)
		{
			...
			switch(argv[i-1].charAt(1))
			{
				...
				case 't': scale_data_filename = argv[i];	break;
				default:
		...
		/* pass 3: scale */
		if(scale_data_filename!=null)
		{
		DataOutputStream DataOutput = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(scale_data_filename)));
		while(readline(fp) != null)
		{
			int next_index = 1;
			double target;
			...
			StringTokenizer st = new StringTokenizer(line," \t\n\r\f:");
			target = Double.parseDouble(st.nextToken());
			output_target(DataOutput,target);
			while(st.hasMoreElements())
			{
				index = Integer.parseInt(st.nextToken());
				value = Double.parseDouble(st.nextToken());
				for (i = next_index; i<index; i++)
					output(DataOutput,i, 0);
				output(DataOutput,index, value);
				next_index = index + 1;
			}
